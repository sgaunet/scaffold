...
// isolating the env/config/default tiers (the flag tier was removed when
// generate became interactive-only).
...
func TestBuildProfile_UnsetConfigFallsThroughToDirName(t *testing.T) {
	dir := emptyDir(t)
	p := buildProfile(dir, config.Config{})
...
func TestBuildProfile_ConfigDockerBooleans(t *testing.T) {
	...
	on := buildProfile(dir, config.Config{
	...
	off := buildProfile(dir, config.Config{Docker: boolp(false)})
	if off.Docker {
...
func TestBuildProfile_PlatformNoneNormalizes(t *testing.T) {
	dir := emptyDir(t)
	p := buildProfile(dir, config.Config{Platform: strp("none")})
